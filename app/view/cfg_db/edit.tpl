...
{% block title %}编辑数据库连接{% endblock %}
{% block head %}
    {{ super() }}
{% endblock %}
 {% block moduletitle %}<h5>数据库配置 <small>添加数据库</small></h5>{% endblock %}
{% block content %}
<form action="update?DB_KEY={{ cfg_db_single.DB_KEY }}" method = 'post'  id="form_cfg_db" class="form-horizontal m-t">
    <div class="form-group">
        <label for="DB_KEY" class="col-sm-3 control-label">数据库编号</label>
        <div class="col-sm-8">
            <input id="DB_KEY" disabled name="DB_KEY" value="{{ cfg_db_single.DB_KEY }}" class="form-control" type="text"/>
        </div>
    </div>
    <div class="form-group">
        <label for="DB_NAME" class="col-sm-3 control-label">数据库名称</label>
        <div class="col-sm-8">
            <input id="DB_NAME" name="DB_NAME" value="{{ cfg_db_single.DB_NAME }}" class="form-control" type="text" />
        </div>
    </div>
    <div class="form-group">
        <label for="DB_VERSION" class="col-sm-3 control-label">数据库类别</label>
        <div id="version" class="col-sm-8">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-8">
            <input id="DB_IP" name="DB_IP" value="{{ cfg_db_single.DB_IP }}" class="form-control" type="text" />
        </div>
    </div>
    <div class="form-group">
        <label for="DB_USERNAME" class="col-sm-3 control-label">数据库用户</label>
        <div class="col-sm-8">
            <input id="DB_USERNAME" name="DB_USERNAME" value="{{ cfg_db_single.DB_USERNAME }}"  class="form-control" type="text" />
        </div>
    </div>
    <div class="form-group">
        <label for="DB_PWD" class="col-sm-3 control-label">数据库密码</label>
        <div class="col-sm-8">
            <input id="DB_PWD" class="form-control" value="{{ cfg_db_single.DB_PWD }}" type="password"  name="DB_PWD" />
        </div>
    </div>
    <div class="form-group">
        <label for="DB_MARK" class="col-sm-3 control-label">备注</label>
        <div class="col-sm-8">
            <textarea id="DB_MARK" class="form-control"  name="DB_MARK" >{{ cfg_db_single.DB_MARK }}</textarea>
        </div>
    </div>
    <div class="form-group">
...
$.validator.setDefaults({
    ...
    $().ready(function() {
         $('#version label input').each(function(){
                var item=$(this)[0]; 
                if(item.value=="{{ cfg_db_single.DB_VERSION }}")
                {
                    item.checked=true;
                }else{
                    item.checked=false;
